<html>

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>House of XP</title>
    <meta name="description" content="">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="icon" type="image/x-icon" href="images/Layer 1.png">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-icons/1.10.2/font/bootstrap-icons.min.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/css/all.min.css" />
    <link rel="stylesheet" href="./css/index.css">
    <meta name="google-site-verification" content="IsIGXNMNAZ6SDfS4KHCDNQd0jr7I0vj-YT0tDVlAoE4" />
</head>

<body>
    <header>
        <nav class="navbar navbar-expand-lg w-100 position-fixed top-0">
            <div class="container">
                <a class="navbar-brand" href="/"><img src="./images/logo.png" alt=""></a>
                <button class="navbar-toggler d-none" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                <i class="bi bi-list"></i>
                </button>
                <input id='add-item-dropdown' type='checkbox'>
                <label for='add-item-dropdown' class="l-bel d-md-none"><i class="bi bi-list"></i></label>
                <div class="menu add-item-dropdown">
                <ul style="font-size:35px" class="navbar-nav ms-lg-auto mb-2 mb-lg-0">
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href="blogs.php">Blogs</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" 
                            href="service">Services</a>
                        </li>
                        
                        <!--<li class="nav-item">-->
                        <!--    <a class="nav-link active" aria-current="page" href="service">Services</a>-->
                        <!--</li>-->
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href="about.php">About</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href="contact.php">Contact</a>
                        </li>
                    </ul>
                </div>
                <div class="collapse py-5 px-3 p-lg-0 navbar-collapse" id="navbarSupportedContent">
                    <!--<div class="connect-section d-flex flex-column">-->
                    <!--    <p style="font-size:18px" class="mb-0 py-1"><a class="text-decoration-none" href="mailto:hello@houseofxp.com"><span class="me-2"><i class="bi bi-envelope"></i></span>hello@houseofxp.com</a>-->
                    <!--    </p>-->
                    <!--</div>-->
                    <ul style="font-size:35px" class="navbar-nav ms-lg-auto mb-2 mb-lg-0">
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href="blogs.php">Blogs</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" 
                            href="service.php">Services</a>
                        </li>
                        
                        <!--<li class="nav-item">-->
                        <!--    <a class="nav-link active" aria-current="page" href="service">Services</a>-->
                        <!--</li>-->
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href="about.php">About</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href="contact.php">Contact</a>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>
    </header>
